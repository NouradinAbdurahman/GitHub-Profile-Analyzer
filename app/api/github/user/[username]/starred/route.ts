...
export async function GET(request: NextRequest, { params }: { params: { username: string } }) {
  // Fix for Next.js dynamic API routes - use proper destructuring
  ...
  try {
    ...
    // Use the specific user's starred endpoint
    const url = `https://api.github.com/users/${username}/starred?per_page=${perPage}&sort=${sort}&direction=${direction}`
    ...
    return NextResponse.json({ error: "Failed to fetch starred repositories" }, { 
      status: 500,
      headers: {
        'Cache-Control': 'public, max-age=60', // Cache errors for 1 minute
        'Content-Type': 'application/json'
